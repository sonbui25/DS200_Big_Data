...
def run_migration(conn: Any, dry_run: bool = False) -> None:
	...
	# ── 0. Pre-flight: count existing rows ────────────────────────────────
	cur.execute("SELECT COUNT(*) FROM comment_embeddings")
	old_count = cur.fetchone()[0]
	logger.info("Existing rows in comment_embeddings: %s", f"{old_count:,}")

	if old_count == 0:
		logger.warning("Table is empty. Nothing to migrate.")

	cur.execute("SELECT MAX(chunk_id) FROM comment_embeddings")
	...
	# ── 4. Copy data ─────────────────────────────────────────────────────
	logger.info("Step 4: Copy data from old table to partitioned table")
	cur.execute("""
	...
	""")
	copied = cur.rowcount
	logger.info("  Copied %s rows", f"{copied:,}")
	conn.commit()
